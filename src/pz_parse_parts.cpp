...
// [[Rcpp::export]]
List split_decimal_degree(float& x, const std::string& fmt = "dms") {
...
DataFrame pz_parse_parts_lat(CharacterVector& x) {
  const int n = x.size();
  IntegerVector deg(n, 0); // IntegerVector.push_back copies the whole vector a each pushback
  ...
  for (int i=0; i < n; ++i) {
    ...
    List parts = split_decimal_degree(out); // passed as a reference.
    deg(i) = parts[0];
    min(i) = parts[1];
    sec(i) = parts[2];
  }
